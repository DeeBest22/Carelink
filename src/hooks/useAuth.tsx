...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      if (sessionUser) {
        fetchProfile(sessionUser.id).then((prof) => {
          if (mounted) setProfile(prof);
        });
      } else if (mounted) {
        setProfile(null);
  ...
  const signUp = useCallback(async (email: string, password: string, role: Role, fullName: string): Promise<SignUpResult> => {
    ...
    // Only create the profile when a session exists (email confirmation disabled).
    // If confirmation is required, the profile is created lazily after verifying.
    if (data.session) {
      if (role === 'provider') {
        const { data: existing } = await supabase
          ...
          .maybeSingle();
        if (existing) {
          await supabase.from('profiles').update({ auth_id: newUser.id }).eq('id', existing.id);
        } else {
          await supabase.from('profiles').insert({ auth_id: newUser.id, role: 'provider', full_name: fullName.trim(), first_name: firstName, email: normalizedEmail });
        }
      } else {
        const { data: prof, error: perr } = await supabase
          .from('profiles')
          .insert({ auth_id: newUser.id, role: 'patient', full_name: fullName.trim(), first_name: firstName, email: normalizedEmail, insurance: 'AXA Mansard Health', primary_doctor: 'Dr. Chinedu Adeyemi' })
          .select('id')
          .maybeSingle();
